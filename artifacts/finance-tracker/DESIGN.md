...
- Floating chrome (bottom nav) is the primary surviving use of liquid glass: `backdrop-filter: blur(24px) saturate(1.8)` over a translucent card-color fill. Two smaller exceptions also use `backdrop-filter`: the biometric PIN modal's card, and the Expense/Income toggle inside the category form — both narrow, high-frequency controls where the glass reads as chrome rather than as a content card.
...
The palette has two layers: a small set of cross-app constants, and two 12-color category ramps (expense, income) that the user assigns per-category and that the treemap/pills render at full saturation.
...
### Secondary — Expense category ramp (violet→red→amber)
Twelve colors, tier-500 saturation (matched to the income ramp's tier on purpose), running violet → red → amber. Assigned by the user per-category via the Categories color picker; rendered flat (no gradient) with white text on the treemap.
- **Violet** (`#8B5CF6`) · **Lilac** (`#C084FC`) · **Fuchsia** (`#D946EF`) · **Orchid** (`#E879F9`) · **Pink** (`#EC4899`) · **Cherry** (`#F43F5E`) · **Blush** (`#FB7185`) · **Red** (`#EF4444`) · **Amber** (`#FBBF24`) · **Honey** (`#F59E0B`) · **Gold** (`#EAB308`) · **Mustard** (`#CA8A04`)
- An earlier iteration excluded orange/yellow entirely; the warm tail (Amber → Mustard) was deliberately reopened later to round the ramp out to 12. Text-on-color still resolves per-swatch via `readableTextColor()`, so the added warm tones don't bypass the contrast check.

### Secondary — Income category ramp (lime→navy)
- **Lime** (`#84CC16`) · **Green** (`#22C55E`) · **Teal** (`#14B8A6`) · **Turquoise** (`#06B6D4`) · **Sky** (`#0EA5E9`) · **Blue** (`#3B82F6`) · **Indigo** (`#6366F1`) · **Grape** (`#A855F7`) · **Cyan** (`#22D3EE`) · **Ocean** (`#0284C7`) · **Royal** (`#2563EB`) · **Navy** (`#1E40AF`)
- These need *more* legibility help under white text than expense does — see the Named Rule below.
...
Floating liquid-glass pill: `border-radius: 999px`, `backdrop-filter: blur(24px) saturate(1.8)`, translucent card-color fill, hairline border. The largest and most visible glass surface in the app — two smaller, narrower controls also use `backdrop-filter` (the biometric PIN modal card, the category form's Expense/Income toggle), but the bottom nav is still the one meant to read as persistent chrome rather than a momentary control.
